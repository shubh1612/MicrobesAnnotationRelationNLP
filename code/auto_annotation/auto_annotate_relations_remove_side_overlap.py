...
with open('../training_set/extra/ann_list.txt', 'r') as filel:
...
for j in file_names:
	...
	for i in range(1,len(sort_end)):
		if sort_end[i][2] == prev[2]:
			...
			# for ma in rel_list:
			# 	print(ma.string_rel())
			# 	if ma.contains('T'+ str(sort_end[i][0])):
			# 		rel_list.remove(ma)
	...
	fp = open('../training_set/corrected_articles/'+search_dest+j, 'w', encoding='utf-8')
