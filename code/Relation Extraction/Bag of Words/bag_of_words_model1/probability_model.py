...
def make_relation(x, y, words, entity_name, count):
	ind1 = int(x[1:])
	ind2 = int(y[1:])
	ind3 = words.index(entity_name[ind1])
	ind4 = words.index(entity_name[ind2])
	prob = 0
	print(ind3,ind4)
...
### open the file containing the names and load them in a list
	...
	with open('../training_set/extra/fileneg_list.txt', 'r') as file2:
	...
	for i in filepos_names:
		...
		for x in range(1,len(datapos)-1):
			if(datapos[x] == prev):
				...
				entity_prob.append(math.log10(count/len(datapos)))
				prev = datapos[x][:-1]
				count = 1
	...
	for i in fileneg_names:
		entity_name = []
		entity_prob = []
		with open('../training_set/'+i, 'r', encoding='utf-8') as myfile:
		...
		for x in range(1,len(dataneg)-1):
			if(dataneg[x] == prev):
				...
				entity_name.append(prev)
				entity_prob.append(math.log10(count/len(dataneg)))
				prev = x
				count = 1
		if(prev == dataneg[len(dataneg)-1]):
			...
			entity_name.append(prev)
			entity_prob.append(math.log10(count/len(dataneg)))
			prev = datapos[len(dataneg)-1]
			count = 1			
		entity_name.append(prev)
		entity_prob.append(math.log10(count/len(data)))
...
for j in file_names:
	...
	with open('../training_set/trn/'+j, 'r', encoding='utf-8') as myfile:
		data=myfile.read().replace('\n',' ')
	fp = open('../training_set/trn/'+j[:-4]+'.ann', 'w', encoding='utf-8')
	...
	for sentence in sent_tokenize(data):
		...
		for entity in l:
			for obj in entity:
				...
				if (obj in lowered_sent and obj[0:len(obj)] == lowered_sent[0:len(obj)])!= False:
					...
					make_relation_list(entity, entity_num, micr, envr, subs, prop, proc, nutr, enz, entity_name, sentence[start : start+len(obj)])
		...
		for x in enz:
			for y in proc:
				if(make_relation(x, y, words, entity_name, 7)):
					rel_num += 1
